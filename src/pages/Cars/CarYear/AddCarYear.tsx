...
const AddCarYear: React.FC<DialogContainerProps> = ({
  isOpen,
  onClose,
  formValues,
}) => {
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (data) => {
      const res = await axios.post(`/createCarYear`, data);
      return res;
    },
  });
    ...
    if (formValues?.id) {
      Update(data, {
        onSuccess() {
          ...
          onClose();
          queryCliet.refetchQueries({ queryKey: ["get-CarYear"] });
          reset({
            name: '',
          });
        },
